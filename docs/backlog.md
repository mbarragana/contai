...
| **D71 — caminho Groq da extração nunca foi exercitado contra a API real** (só mockado nos 90 testes unitários; Gate 3 do `CONTAI-052` rodou sem `GROQ_API_KEY`, viu só o caminho Gemini) | `69-2026-09-25-contai-052-entregue.md` → depois que a chave existir na Vercel, conferir no log de produção que aparece `origem: "texto+groq"` ao menos uma vez |
