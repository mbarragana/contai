...
| **D70 — `fetch()` da extração Gemini não tem timeout explícito** (`AbortSignal.timeout` ausente) nem a rota `app/api/extrair-documento` declara `maxDuration`: resposta pendurada não aciona o retry (que só cobre quem *responde* com erro) | `67-2026-09-24-incidente-extracao-gemini.md` → achada pelo `cto-obra` no Gate 2 do retry; **fechamento previsto pelo `CONTAI-052`** (critério 7: `AbortSignal.timeout()` nos dois provedores + `maxDuration=60` na rota), confirmar no Gate 2 dele antes de dar como resolvida |
