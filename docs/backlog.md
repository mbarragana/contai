...
| **D79 — `compromisso.documento_origem_id` é write-only**: o Mateus afirma no agendamento (PIX, boleto ou compra no cartão) qual nota originou o compromisso, mas nenhuma quitação (`fatura_desembolso_gravar`/`fatura_alocar` do cartão, `quitarCompromisso` do PIX/boleto) propaga esse vínculo para `pagamento_documento` — achada pelo `cto-obra` ao investigar por que a compra no cartão não herdava a nota | `78-2026-09-26-cartao-nao-herda-documento.md`. Parecer do `contador` (`docs/pareceres/2026-09-26-replicar-vinculo-documento-quitacao.md`): replicar automaticamente é seguro (não é heurística, é persistir afirmação humana já feita), com 5 condições de guarda. Paga **parcialmente** pelo `CONTAI-064` (herança na captura); paga por completo pelo `CONTAI-065` (propagação na quitação, com migration) |
