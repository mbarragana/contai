...
## Campos
- `meio` (s1) — 3 botões: "PIX" | "Boleto" | "Cartão" — obrigatório — decide o branch junto com a data; em **cartão a data não decide nada** e o registro é sempre agendamento — **SEM DEFAULT — campo fiscal**: o mock nascia em PIX (Dúvidas 1, que já assumia SEM DEFAULT e pedia decisão explícita), e foi essa pré-seleção que virou a D44 — decidida pelo CONTAI-032
...
- `cData` — "Data em que o dinheiro saiu *" (s10) — `type=date` — obrigatória — **recusa data futura** e trava o CTA — DEFAULT DECLARADO: a data prevista do agendamento, **editável**; a prevista é descartada na gravação (ver Dúvidas 2). É o falso positivo nomeado do CONTAI-034, critério 12 — default legítimo **porque está declarado nesta linha**, e não porque alguma regra geral o permita
- `cValor` — "Valor efetivamente pago *" (s10) — `number step=0.01` — obrigatório — compara com o previsto e abre os blocos de maior/menor — SEM DEFAULT — campo fiscal.
  ⚠️ **D65, decidida pelo `contador` no Gate 2 do CONTAI-034 (2026-09-21).** O código pré-preenchia este campo com o saldo do agendamento, e por um dia esta linha foi dobrada para descrever isso — o anti-padrão inverso da D44, onde foi o CÓDIGO que se corrigiu para bater com o spec. Fundamento: `cValor` é o que foi PAGO e vira custo de aquisição sob regime de caixa; o previsto **nunca é o número que conta**, como o próprio card acima do campo já diz. O campo gêmeo do registro direto (`fValor`, `/adicionar/pagamento`) é SEM DEFAULT pelo mesmo fato. O código foi corrigido
- `cEncargos` — "Juros e multa por atraso *" (s10) — `number step=0.01` — obrigatório **só quando pago > previsto** — o excedente não identificado vira "sem explicação" e é marcado para revisão — **SEM DEFAULT — campo fiscal**: o mock trazia 0.00 e a Dúvidas 1 já assumia SEM DEFAULT (0,00 pré-digitado afirma "não houve juros", que é fato que ninguém conferiu)
